...
def create_account():
    ...
    try:
            ...
            new_user = User(first_name=first_name, last_name=last_name, email=email, hashed_password=hashed_password, join_date=signup_date)
    ...
    return render_template('verifyidentity.html')
...
def download_link():
    ...
    for item in ordered_item:
        ...
        new_order = Order(item_title=item.book_name, book_file=bought_book.pdf_filename, item_author=item.book_author, total_price=TotalPrice(), user_id=session['id'], item_id=item.item_id, book_image=item.book_coverimage)
...
def order_shelf():
    ...
    orders = Order.query.filter_by(user_id=session['id'])
